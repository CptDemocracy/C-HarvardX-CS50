...
const char* startsWith(const char* str, const char* substr, size_t start);
...
char* indexes(const char* path)
{
	...
	fileName = "/index.php";	
	if (!(uri = (char*) malloc(strlen(path) + strlen(fileName) + 1))) {
		return NULL;
	}
	strcpy(uri, path);
	strcat(uri, fileName);
	...
	fileName = "/index.html";
	if( !(uri = realloc(uri, strlen(path) + strlen(fileName) + 1))) {
		return NULL;
	}
	strcpy(uri, path);
	strcat(uri, fileName);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
	...
	size_t bufferSize = 4;
	if (!(*content = (BYTE*) malloc(sizeof(BYTE) * bufferSize))) {
    ...
    while (fread((*content) + bytesRead, sizeof(BYTE), 1, file) == 1) {
        if (bytesRead >= bufferSize) {
			bufferSize = (bufferSize != 0) ? bufferSize * 2 : 4;
			if (!(*content = realloc(*content, sizeof(BYTE) * bufferSize))) {
				error(500); // Server Error
				return false;
			}			
		}
	...
	// trim unused bytes in the buffer
	BYTE* trimmedBuffer;
	if (!(trimmedBuffer = (BYTE*) malloc(sizeof(BYTE) * bytesRead))) {
		error(500); // Server Error
		return false;
	}
	memcpy(trimmedBuffer, *content, bytesRead);
	free(*content);

	*content = trimmedBuffer;
	*length  = bytesRead;
    return true;
}
...
const char* lookup(const char* path)
{
	...
	// skip back to the end of the path past all the whitespace
	while (path[pathLen - 1] == ' ' && path + pathLen > path + 1)
		--pathLen;

	const char* ext;

	ext = ".css";
	if (strncasecmp(path + pathLen - strlen(ext), ext, strlen(ext)) == 0) {
		return "text/css";
	} 
...
bool parse(const char* line, char* abs_path, char* query)
{
	...
	size_t absPathCount = 0;
	if (absPathCount >= LimitRequestLine) {
		error(500); // Server error
		return false;
	}
	abs_path[absPathCount] = '/';
	++absPathCount;
	if (!isblank(*c)) {
		while (*c != '.' && !isblank(*c)) {
			if ((*c == '\0') || (*c != '/' && !isalnum(*c))) {
	...
	if (*c == '?') {
		...
		while (*c != '\0' && !isblank(*c)) {
			...
			if (queryCount >= LimitRequestLine) {
				error(500); // Server error
				return false;
			}
			query[queryCount] = *c;
	...
	++c;
	if ( !(c = startsWith(c, "HTTP/", 0))) {
		error(400); // Bad Request
		return false;
	}
	if ( !(c = startsWith(c, "1.1", 0))) {
