...
$(function() {
    ...
    origLat = 41.3184;  // New Haven latitude
    origLng = -72.9318; // New Haven longitude

    // get urlParams' size
    var size = 0; 
    for (var key in urlParams) 
    { 
        if (urlParams.hasOwnProperty(key)) ++size; 
    }
    if (size > 0) {
        origLat = parseFloat(urlParams["lat"]);
        origLng = parseFloat(urlParams["lng"]);
    }
    ...
    var options = {
        center: {lat: origLat, lng: origLng},
...
function addMarker(place)
{
    ...
    $.getJSON("articles.php", parameters)
        .done(function(data, textStatus, jqXHR) {
            ...
            // like
            content += '<form action="like.php" method="post">'
                ...
                + '<input type="hidden" name="admin_name1" value="'+place.admin_name1+'"/>'
                + '<input type="hidden" name="country_code" value="'+place.country_code+'"/>'
...
function removeMarkers()
{
    ...
    markers.length = 0; 
    // woah, I didn't know that, I thought you had to explicitly: markers = [];
}
